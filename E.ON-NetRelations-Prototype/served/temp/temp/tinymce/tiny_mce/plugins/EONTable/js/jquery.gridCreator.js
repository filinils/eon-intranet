(function($){
	...
	$.gridCreator = function(element, options) {
		...
		plugin.init = function() {
			...
			for (row = 0; row < maxRows; row++) {									
				for (col = 0; col < maxCols; col++) {
					var $gridItem = $('<div>');
					$gridItem.addClass(plugin.settings.gridClass).attr('row', row).attr('col', col);
		...
		var onMouseUp = function() {
			mouseDown = false;
			if(plugin.settings.valueHolder !== null){
				$('#'+plugin.settings.valueHolder).val(2);
			}
		};
		...
		var updateGrid = function(target) {
			...
			if (itemRow === undefined || itemCol === undefined) {
				return;
			}
			if (prevRow === itemRow && prevCol === itemCol) {
				return;
			}
			var maxCols = plugin.settings.cols;
			var maxRows = plugin.settings.rows;
