	...
	$.extend($.fn, {
			...
			if(!jQueryFormUtils.ignoreInput($element.attr('name'), elementType, config)) {
				...
				if(validation === true) {
					...
					if( elementErrMsgObj != null) {
						...
						$element.prev().append('<span class="jquery_form_error_message"><strong>' + $element.data('validation-error-text') + '</strong></span>');	
					}
		...
		validate : function(language, settings) {
			...
			var config = {
				...
				errorElementClass : 'error', // Class that will be put on elements which value is invalid
				borderColorOnError : '#d30d01', // Border color of elements which value is invalid, empty string to not change border color
				...
				validationErrorMsgAttribute : 'data-validation-error-msg', // define custom err msg inline with element
				errorMessagePosition : 'over', // Can be either "top" or "element"
			...
			$form.find('input,textarea,select').each(function() {
				...
				if (!jQueryFormUtils.ignoreInput($el.attr('name'), elementType, config)) {
		            ...
		            // Added by NetRelations
		            if($el.hasClass('wysiwyg')){
		               if($.trim(tinyMCE.get($el.attr('id')).getContent()) !== ''){
		                   return true;
		                }
		            }
		   ...
		   // if(jQueryFormUtils.defaultBorderColor == config.borderColorOnError)
			...
			//
			$form.find('.error').removeClass('error').find('strong').remove();
			...
			if (errorInputs.length > 0) {
				...
				for (var i = 0; i < errorInputs.length; i++) {
					var parent;
					if(errorInputs[i].attr('type') === 'radio' || errorInputs[i].attr('type') === 'checkbox'){
						parent = errorInputs[i].parent().parent().parent().parent();
						parent.addClass('error');
						errorInputs[i].parent().parent().parent().find('legend').after('<span class="jquery_form_error_message"><strong>' + errorInputs[i].data('validation-error-text') + '</strong></span>');	
					}else{
						parent = errorInputs[i].prev();	
						errorInputs[i].parent().addClass('error');
						parent.append('<span class="jquery_form_error_message"><strong>' + errorInputs[i].data('validation-error-text') + '</strong></span>');	
					}
